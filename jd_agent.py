...
class JDResumeAgent:
    ...
    def extract_skills_from_jd_from_jd(self, jd_text: str) -> List[str]:
        """
    ...
    def _normalize_skills(self, skills: List[str]) -> Set[str]:
        """
        Normalize extracted skills to standard forms.
        
        Args:
            skills (List[str]): Raw extracted skills
            
        Returns:
            Set[str]: Normalized skills
        """
        normalized = set()
        ...
        for skill in skills:
            ...
            # If no match found, keep original if it looks like a skill
            if not found and len(skill_lower) > 2 and len(skill_lower.split()) <= 3:
                # Filter out obvious non-skills
                if not any(stop_word in skill_lower for stop_word in ['the', 'and', 'or', 'with', 'for', 'to', 'of']):
                    normalized.add(skill_lower)
